...
|---|---|---|
| **Hub** (`index.html`) *(unverified)* | Load the page. Confirm 8 game cards render with thumbnails/photos, not blank tiles. Tap "Cloudhopper" (top-left card). Confirm it navigates to `cloudhopper.html` and the game loads. Tap the Bramblewood link top-right inside that game. Confirm you land back on the hub. | No debug handle on the hub. Check visually: `document.querySelectorAll('.gamecard').length === 8` in devtools, and that none of the 8 `href`s 404. |
...
|---|---|---|
| Console errors | Open devtools console, load each of the 9 pages fresh | No red errors on load or during a short play session |
