...
| **Puffball** (`puffball.html`) | Open the game, pick night 1. Drop a puffball, count the lit squares, move away, let it pop, catch a mole or clear a pot. | `window.__puff.debugState()`: `phase` is `play`, `puffs` goes from 0 to 1 while it's ticking and back to 0 after it pops, `reach` matches the number printed on the cap, and either `knocks` increases (a mole caught) or `potsLeft` decreases. |
| **Moonshot** (`moonshot.html`) | Open the game. Tap a small tank, then a small engine, so the stack reads engine-under-tank, and watch the balance tip to "The push wins". Tap Launch, hold the left or right thumb pad when the bubble drifts off centre, and let the fuel run out. | `window.__moon.debugState()`: `phase` goes `build` → `count` → `fly` → `coast` → `done`. During `fly`, `altKm` and `vUp` both rise and `stage` increments each time a spent stage drops. At `done`, `topKm` is the height reached and `best` is the highest ladder rung recorded. `tilt` should stay under about 0.3 while you are steering. Then go back and add four boosters: the gauge under the balance should drop to the amber end and read "Die boosters vangen veel wind", and in flight the nose should glow and streaks should run past the body through the thick air. |
| **Millstream** (`mill.html`) | Open the game, pick valley 1. Dig a channel from spring to field with a plausible slope and let water flow to a mill wheel. | `window.__mill.debugState()`: `phase` moves `levels` → `play` → `won` (or `failed` if the channel floods/stalls), `water` (total water) rises above 0, and `wasted` should not run away while you're actively directing flow. |
